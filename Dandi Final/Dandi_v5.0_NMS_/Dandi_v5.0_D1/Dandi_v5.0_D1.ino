...
void SendData_rawdata(int16_t *pAcX, int16_t *pAcY, int16_t *pAcZ, int16_t *pGyX, int16_t *pGyY, int16_t *pGyZ, int  start_index ) {
  ...
  if (conn1 == 1)                       // After succesful connection sending data to client
  {
    ...
    //Request
    client.println("POST /dandi/dandi_device_3/dandi_test_post_new.php HTTP/1.1");
...
int connect_to_wifi() {
  ...
  while ((WiFi.status() != 3) && returnbit == 1)
  {
    Serial.println(WiFi.status());
...
void Serialnumber() {
  ...
  if (conn7 == 1) {
    ...
    client7.print("GET /dandi/dandi_device_3/dandi_test_post_new.php?Fn="); client7.print(Fn);
